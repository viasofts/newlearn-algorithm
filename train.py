...
from utils.plots import plot_results_csv
import shutil
...
class Yolov5:
    ...
    def train(self, hyp, opt, device, tb_writer=None, wandb=None):
        ...
        opt_yaml_path = os.path.join(save_dir, "opt.yaml")
        ...
        for epoch in range(
            ...
            end_time = time.time()
            cur_epoch = epoch + 1
            expected_time = (end_time - start_time) * (epochs - cur_epoch)
    ...
    def main(self):
        ...
        opt.log_imgs = 16
        save_path = os.path.join(self.project, "exp")
        ...
        # Evolve hyperparameters (optional)
            ...
            for _ in range(300):  # generations to evolve
                if Path("evolve.txt").exists():  # if evolve.txt exists: select best hyps and mutate
                    ...
                    x = x[np.selfort(-fitness(x))][:n]  # top n mutations
                    w = fitness(x) - fitness(x).min()  # weights
                    if parent == "single" or len(x) == 1:
                        # x = x[random.randint(0, n - 1)]  # random selection
                        x = x[random.choices(range(n), weights=w)[0]]  # weighted selection
                    elif parent == "weighted":
                        x = (x * w.reshape(n, 1)).sum(0) / w.sum()  # weiOghted combination
