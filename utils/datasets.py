...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            if not ret_val:
                img0 = self.previous_img

            self.previous_img = img0
            # if not ret_val:
            #    self.count += 1
            #    self.cap.release()
...
def img2label_paths(img_paths):
    ...
    for x in img_paths:
        ...
        count = 0
        parse_datas = x.split(".")
        parse_len = len(parse_datas)

        for parse_data in parse_datas:
            if count == (parse_len - 1):
                label_path += "txt"
            else:
                label_path += parse_data + "."

            count += 1
        label_list.append(label_path.replace(sa, sb, 1))
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def cache_labels(self, path=Path("./labels.cache"), prefix=""):
        ...
        for i, (im_file, lb_file) in enumerate(pbar):
            ...
            pbar.desc = f"{prefix}Scanning '{path.parent / path.stem}' for images and labels... " f"{nf} found, {nm} missing, {ne} empty, {nc} corrupted"
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        results = re.findall(pattern, path)
        if len(results) == 0:
...
def load_mosaic(self, index):
    ...
    # Augment
    img4, labels4 = random_perspective(img4, labels4, degrees=self.hyp["degrees"], translate=self.hyp["translate"], scale=self.hyp["scale"], shear=self.hyp["shear"], perspective=self.hyp["perspective"], border=self.mosaic_border)  # border to remove
...
def load_mosaic9(self, index):
    ...
    # Augment
    img9, labels9 = random_perspective(img9, labels9, degrees=self.hyp["degrees"], translate=self.hyp["translate"], scale=self.hyp["scale"], shear=self.hyp["shear"], perspective=self.hyp["perspective"], border=self.mosaic_border)  # border to remove
